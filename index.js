...
function addGamesToPage(GAMES_JSON) {
    ...
    for (let i = 0; i < GAMES_JSON.length; i++) {
        ...
        gamesDiv.innerHTML = `
        <img class="game-img" src="${GAMES_JSON[i].img}"> </img>
        <p>${GAMES_JSON[i].name}</p>
        <p>${GAMES_JSON[i].description}</p>
        <p>Backers: ${GAMES_JSON[i].backers}</p>
        `;
        
        // append the game to the games-container
        gamesContainer.append(gamesDiv);
    }
...
descStr.innerHTML = `A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${totalGames} games. 
Currently, ${unfundedGames} ${unfundedGames == 1 ? "game remains" : "games remain"} 
unfunded. We need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container
descriptionContainer.append(descStr);
